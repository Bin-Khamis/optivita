...
export function normalizeE164(phone: string, selectedDialCode: string = ""): string {
  ...
  for (const c of sorted) {
    if (cleaned.startsWith(c.dial_code)) {
      ...
      // Remove leading zero after country code if necessary
...
export function validatePhone(phone: string): boolean {
  ...
  // E.164 phone numbers have between 7 and 15 digits total
  return phone.startsWith("+") && digits.length >= 7 && digits.length <= 15;
}
