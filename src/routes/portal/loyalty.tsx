...
function CustomerLoyalty() {
  ...
  const clientLedger = ledger.filter((l: any) => {
    ...
    return (
      ...
      String(ledgerEnrollId).trim() === String(customer.enrollmentId).trim()
    );
  });
  ...
  const handlePrintSection = (elementId: string, isPdf = false) => {
    ...
    if (isPdf) {
      toast.info(
        "Opening PDF print layout. Please select 'Save as PDF' or 'Microsoft Print to PDF' in the destination dropdown.",
      );
    }
  ...
  const handleRedeemRewardSubmit = async () => {
    ...
    if (isWebhookOffline(webhookUrl)) {
      ...
      setTimeout(() => {
        ...
        toast.success(
          `Redeemed successfully: ${redeemingReward.RewardName}! Code: OPT-CLAIM-${Math.floor(100000 + Math.random() * 900000)}`,
        );
    ...
    try {
      ...
      if (result.status === "success") {
        toast.success(
          `Successfully redeemed ${redeemingReward.RewardName}! Confirmation sent via WhatsApp.`,
        );
        refreshData();
        setRedeemingReward(null);
      } else {
  ...
  const BarcodeRenderer = ({ code }: { code: string }) => {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      lines.push(
        <div
          key={i}
          className={`h-10 ${isBlack ? "bg-slate-800" : "bg-transparent"} ${width} shrink-0`}
        />,
      );
    }
    ...
    return (
      <div className="flex flex-col items-center gap-1.5 mt-2.5 p-2 bg-white rounded-lg border border-slate-200/40 w-fit mx-auto">
        <div className="flex gap-[1.5px] items-center h-10 overflow-hidden bg-white">{lines}</div>
        <span className="font-mono text-[7px] font-bold text-slate-500 tracking-[3px] uppercase">
          {code}
        </span>
      </div>
    );
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Review your digital membership card, ledger transactions, and redeem rewards.
          </p>
        </div>
        ...
        <div className="p-12 rounded-[28px] bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 text-center space-y-4 shadow-soft">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto leading-relaxed">
            Your Digital Loyalty Membership Card and Rewards Catalogue will become active as soon as
            your joining confirmation is approved by your admin coach.
          </p>
        </div>
      </div>
    );
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[24px] p-6 shadow-soft space-y-6">
            ...
            >
              ...
              {!cardFlip ? (
                ...
                <div className="h-full flex flex-col justify-between relative z-10">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                      <img
                        ...
                        alt="Optivita Logo"
                        className="h-6.5 w-6.5 object-contain filter brightness-0 invert"
                      />
                      <div>
                        <p className="font-display font-black tracking-wider text-sm leading-none">
                          OPTIVITA
                        </p>
                        <p className="text-[7px] text-emerald-250 font-semibold tracking-widest mt-1 uppercase">
                  ...
                  <div>
                    <p className="text-[9px] uppercase tracking-wider text-emerald-250 font-semibold leading-none">
                      Card Holder
                    </p>
                    <p className="font-display font-bold text-lg mt-1 truncate">
                      {customer?.fullName}
                    </p>
                    <p className="text-[8px] text-emerald-100/80 mt-0.5 truncate">
                  ...
                  <div className="flex justify-between items-end border-t border-white/10 pt-3">
                    <div>
                      ...
                      <p className="font-mono text-[9px] font-bold mt-0.5">
                        {customer?.enrollmentId}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-[7px] text-emerald-250/80 uppercase">Points Balance</p>
                      <p className="font-display font-black text-sm text-emerald-350">
                        {points} pts
                      </p>
                    </div>
                  </div>
                ...
                <div className="h-full flex flex-col justify-between relative z-10 text-slate-350 text-[8px] leading-relaxed">
                  <div>
                    <p className="font-bold text-slate-200 text-[9px] uppercase mb-1">
                      Terms & Conditions
                    </p>
            ...
            <p className="text-center text-[10px] text-slate-400">
              Tap loyalty card to view back details and barcode.
            </p>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[24px] p-6 shadow-soft space-y-4">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Earn <strong>+300 loyalty points</strong> automatically when a friend registers using
              your code! They also receive a welcome bonus.
            </p>
            <div className="p-3.5 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex justify-between items-center">
              <span className="font-mono text-xs font-bold text-slate-700 dark:text-slate-300 truncate max-w-[150px]">
                {referralCode}
              </span>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {activeRewards.map((reward: any) => {
                ...
                return (
                  <div
                    key={reward.RewardId}
                    className="p-4 border rounded-2xl bg-slate-50/50 dark:bg-slate-900/40 border-slate-100 dark:border-slate-800 flex flex-col justify-between hover:shadow-soft transition-all duration-200"
                  >
                    <div>
                    ...
                    clientLedger.map((log: any, idx: number) => (
                      ...
                      >
                        <td className="py-3.5 px-3 text-slate-400">
                          {log.Timestamp?.split(",")[0] || "Just now"}
                        </td>
                        <td className="py-3.5 px-3 font-semibold">{log.Activity}</td>
                        <td className="py-3.5 px-3 text-center text-emerald-500 font-bold">
                          +{log["Points Earned"] || 0}
                        </td>
                        <td className="py-3.5 px-3 text-center text-red-500 font-bold">
                          -{log["Points Redeemed"] || 0}
                        </td>
                        <td className="py-3.5 px-3 text-right font-black text-slate-700 dark:text-slate-300">
                          {log["Current Balance"] || 0} pts
                        </td>
                      </tr>
                    ))
                  ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-md p-6 shadow-glow z-10 animate-scale-up">
            ...
            <p className="text-xs text-slate-400 mb-5">
              Would you like to redeem points for this reward?
            </p>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 text-xs leading-relaxed mb-5">
              <p className="font-bold text-slate-800 dark:text-slate-100">
                {redeemingReward.RewardName}
              </p>
              <p className="mt-1">{redeemingReward.Description}</p>
              <p className="mt-3 text-emerald-600 dark:text-emerald-400 font-bold">
                Cost: {redeemingReward.PointsRequired} Points
              </p>
            </div>
