...
function CustomerDashboard() {
  ...
  const progressVal =
    getProgress(clientEnrollment) !== undefined ? Number(getProgress(clientEnrollment)) : 0;
      ...
      {!isConfirmed && (
        <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-900 dark:text-amber-300 font-medium text-xs flex items-center gap-3">
          ...
          <div>
            ...
            <span>
              Your Digital Loyalty Card, Rewards, Invoices, and Account Statements will unlock
              automatically once your program enrollment is confirmed by an admin coach.
            </span>
          </div>
        </div>
      )}
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-sm md:text-base text-emerald-100/90 max-w-xl">
              Track your personalized nutrition progress, schedule clinical reviews, and view your
              account statements.
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-5 shadow-soft flex items-center justify-between">
          <div>
            ...
            <span className="text-[10px] text-slate-400 mt-1 block">
              Tier Status: <strong>{tier}</strong>
            </span>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-5 shadow-soft flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-black text-slate-800 dark:text-slate-100 mt-2">
              {clientAssessment.bmi || "24.6"}
            </p>
            <span className="text-[10px] text-slate-400 mt-1 block">
              Category: <strong>{clientAssessment.bmiCategory || "Normal"}</strong>
            </span>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-5 shadow-soft flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-black text-amber-500 mt-2">
              {clientAssessment.dailyCalories || "2200"} kcal
            </p>
            <span className="text-[10px] text-slate-400 mt-1 block">
              Water: <strong>{clientAssessment.dailyWater || "3.2"} L</strong>
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[24px] p-6 shadow-soft space-y-6">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 px-3 py-1 rounded-full font-bold">
              In Progress
            </span>
          </div>

          <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-950 border">
            <p className="font-display font-bold text-lg text-slate-800 dark:text-slate-100">
              {customer?.programName}
            </p>
            <p className="text-xs text-slate-400 mt-1 leading-normal">
              Precision health tracking plan. Personal clinical coach:{" "}
              <strong>{clientEnrollment["Assigned To"] || "Clinical Lead"}</strong>
            </p>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[24px] p-6 shadow-soft flex flex-col justify-between">
              ...
              {clientAppointments.length > 0 ? (
                clientAppointments.map((apt: any) => (
                  <div
                    ...
                    className="p-3 border rounded-xl bg-slate-50 dark:bg-slate-950 text-xs leading-normal"
                  >
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-slate-900 dark:text-slate-100">
                        Coaching Session
                      </span>
                      <span className="px-2 py-0.5 rounded-full text-[8px] font-black bg-emerald-500/10 text-emerald-600 uppercase">
                        {apt.status}
                      </span>
                    </div>
          ...
          <Link
            ...
            className="w-full text-center py-2.5 border rounded-xl text-xs font-bold hover:bg-slate-50 dark:hover:bg-slate-800 mt-4 block text-slate-600 dark:text-slate-300"
          >
            Book Coach Session
          </Link>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              View active billing statements, generated receipts, and payment history.
            </p>
          </div>
          {isConfirmed && (
            <Link
        ...
        {!isConfirmed ? (
          <div className="p-8 rounded-2xl bg-amber-500/5 border border-amber-500/20 text-center space-y-3">
            ...
            <h4 className="font-bold text-sm text-slate-800 dark:text-slate-200">
              Account Statements & Invoices Locked
            </h4>
            <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
              Financial statements and active invoices will become accessible here as soon as your
              joining confirmation is approved by your admin coach.
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800">
                ...
                <p className="text-xl font-extrabold text-slate-800 dark:text-slate-100 mt-1">
                  SAR {totalBilled.toLocaleString()}
                </p>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-amber-50/50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30">
                ...
                <p className="text-xl font-extrabold text-amber-700 dark:text-amber-400 mt-1">
                  SAR {outstandingBalance.toLocaleString()}
                </p>
              </div>
            </div>
