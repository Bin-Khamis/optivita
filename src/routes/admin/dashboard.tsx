...
function AdminDashboard() {
  ...
  const pendingToday = appointments.filter((a: any) => {
    const status = String(a.status || a.Status || "").toLowerCase();
    const dateStr = String(a.date || a["Appointment Date"] || "").trim();
    return (
      status === "pending" &&
      (dateStr === todayStr ||
        dateStr === todayFormatted ||
        dateStr.includes(todayStr) ||
        dateStr.includes(todayFormatted))
    );
  });
  ...
  const avgHealthScore = assessments.length
    ? Math.round(
        assessments.reduce((acc: number, curr: any) => acc + parseFloat(curr.healthScore || 0), 0) /
          assessments.length,
      )
    : 72;
  ...
  const avgWater = assessments.length
    ? (
        assessments.reduce((acc: number, curr: any) => acc + parseFloat(curr.dailyWater || 0), 0) /
        assessments.length
      ).toFixed(1)
    : "2.8";
  ...
  const avgSteps = assessments.length
    ? Math.round(
        assessments.reduce((acc: number, curr: any) => acc + parseFloat(curr.dailySteps || 0), 0) /
          assessments.length,
      )
    : 8500;
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Real-time indicators across clinical programs and CRM flows
          </p>
        </div>
        <button
          onClick={() => refreshData()}
      ...
      {/* New Appointment Requests (Today's Pending) Widget */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[24px] p-6 shadow-soft text-left space-y-4">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-base leading-none">
              New Appointment Requests (Today's Pending)
            </h3>
          </div>
          <Link
            to="/admin/appointments"
            className="text-xs font-bold text-[#0D4E8A] dark:text-emerald-400 hover:underline flex items-center gap-1"
          >
            Go to Appointments Hub <ArrowRight className="h-3 w-3" />
          </Link>
        </div>

        {pendingToday.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {pendingToday.map((apt: any) => {
              const id = apt["Appointment ID"] || apt.AppointmentId;
              const name = apt.fullName || apt["Customer Name"] || "Client";
              const time = apt["Appointment Time"] || apt.time || "N/A";
              const reason = apt.Reason || apt.reason || "Wellness Consultation";

              return (
                <div
                  key={id}
                  className="p-4 border rounded-2xl bg-amber-500/[0.03] dark:bg-yellow-500/[0.02] border-yellow-200/60 dark:border-yellow-900/30 flex justify-between items-center hover:shadow-soft transition-all"
                >
                  <div className="space-y-1 min-w-0">
                    <p className="font-semibold text-xs text-slate-850 dark:text-slate-200 truncate">
                      {name}
                    </p>
                  ...
                  <Link
                    to="/admin/appointments"
                    className="px-3 py-1.5 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg text-[10px] font-black uppercase tracking-wider"
                  >
                    Action
                  </Link>
                </div>
          ...
          <div className="p-5 border border-dashed rounded-2xl border-slate-200 dark:border-slate-800 text-center text-slate-400 text-xs bg-slate-50/50 dark:bg-slate-950/20">
            No pending appointment requests scheduled for today.
          </div>
        )}
      </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1.5">
                Popularity metrics per medical wellness package
              </p>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[24px] p-6 shadow-soft">
        <h3 className="font-semibold text-base leading-none mb-6">
          Precision Health Performance Targets
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="border border-slate-100 dark:border-slate-800/60 rounded-xl p-4.5">
            ...
            <p className="text-[10px] text-slate-400 mt-2">
              Clinical evaluation calculations average
            </p>
          </div>
          ...
          <div className="border border-slate-100 dark:border-slate-800/60 rounded-xl p-4.5">
            ...
            <div className="w-full bg-slate-100 dark:bg-slate-800 h-2.5 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(parseFloat(avgWater) / 5) * 100}%` }}
              />
            </div>
            <p className="text-[10px] text-slate-400 mt-2">
              Recommended daily water intake average
            </p>
          </div>
          ...
          <div className="border border-slate-100 dark:border-slate-800/60 rounded-xl p-4.5">
            ...
            <div className="w-full bg-slate-100 dark:bg-slate-800 h-2.5 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(avgSteps / 15000) * 100}%` }}
              />
            </div>
            ...
            {appointments.length > 0 ? (
              appointments.map((apt: any) => (
                <div
                  ...
                  className="p-3 border border-slate-100 dark:border-slate-800 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
                >
                  <div className="flex items-center justify-between">
