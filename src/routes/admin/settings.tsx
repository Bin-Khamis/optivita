...
function AdminSettings() {
  ...
  // Tab control state
  const [activeTab, setActiveTab] = useState<"loyalty" | "provider">("loyalty");
  ...
  useEffect(() => {
    if (data?.["Settings"]) {
      const getSetting = (key: string) => {
        const item = data["Settings"].find(
          (s: any) =>
            String(s.Key || s.key || "")
              .trim()
              .toLowerCase() === key.toLowerCase(),
        );
        return item ? String(item.Value || item.value || "") : "";
      };
      setCompanyName(getSetting("Provider_Company_Name") || "OPTIVITA");
      setLogoUrl(getSetting("Provider_Logo") || "/optivita-logo.png");
      setProviderEmail(getSetting("Provider_Email") || "optivita.support@gmail.com");
      setProviderPhone(getSetting("Provider_Phone") || "+965 12345678");
      setProviderWebsite(getSetting("Provider_Website") || "www.optivita.netlify.app");
      setProviderAddress(getSetting("Provider_Address") || "123 Health Street");
      setProviderCity(getSetting("Provider_City") || "Kuwait City");
      setProviderCountry(getSetting("Provider_Country") || "Kuwait");
  ...
  const handleSaveProviderInfo = async (e: React.FormEvent) => {
    e.preventDefault();
      ...
      setTimeout(() => {
        ...
        Object.entries(fields).forEach(([key, val]) => {
          ...
          if (match) {
            ...
            data["Settings"].push({ Key: key, Value: val });
          }
        });
        localStorage.setItem("optivita_crm_cache", JSON.stringify(data));
        toast.success("Provider information updated (offline cache)!");
        refreshData();
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Configure loyalty points weights, rewards catalog items, and company branding details.
        </p>
      </div>
      ...
      <div className="flex gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
        <button
          ...
          className={`px-4.5 py-2 text-xs font-bold rounded-lg border uppercase tracking-wider transition-all duration-200 ${
            activeTab === "loyalty"
              ? "bg-emerald-600 text-white border-emerald-500 shadow-soft"
              : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-950"
          }`}
        >
      ...
      {activeTab === "loyalty" ? (
        ...
        /* Company / Provider Information Tab */
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[24px] p-8 shadow-soft text-left">
          <div className="flex items-center gap-2 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-slate-450 dark:text-slate-400 mt-1.5">
                Manage administrative business details used dynamically across customer invoices and
                receipts.
              </p>
            </div>
          </div>

          <form onSubmit={handleSaveProviderInfo} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. Optivita Health Center"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">
                  <Globe className="h-3 w-3" /> Brand Logo URL
                </label>
                <input
                  type="text"
                  value={logoUrl}
                  onChange={(e) => setLogoUrl(e.target.value)}
                  placeholder="e.g. /optivita-logo.png"
                  className="w-full p-3 text-xs border rounded-xl bg-slate-50 dark:bg-slate-955 border-slate-200 dark:border-slate-800/80 text-slate-850 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  required
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. support@optivita.com"
                  className="w-full p-3 text-xs border rounded-xl bg-slate-50 dark:bg-slate-955 border-slate-200 dark:border-slate-800/80 text-slate-850 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  required
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. www.optivita.health"
                  className="w-full p-3 text-xs border rounded-xl bg-slate-50 dark:bg-slate-955 border-slate-200 dark:border-slate-800/80 text-slate-850 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  required
                />
              </div>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">
                  <Sliders className="h-3 w-3" /> Registration Number (Optional)
                </label>
                <input
                  type="text"
                  value={providerRegistration}
                  onChange={(e) => setProviderRegistration(e.target.value)}
                  placeholder="e.g. REG-10928-KU"
                  className="w-full p-3 text-xs border rounded-xl bg-slate-50 dark:bg-slate-955 border-slate-200 dark:border-slate-800/80 text-slate-850 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                />
              </div>
              ...
              <div className="space-y-1 md:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. Al Hamra Tower, Floor 32"
                  className="w-full p-3 text-xs border rounded-xl bg-slate-50 dark:bg-slate-955 border-slate-200 dark:border-slate-800/80 text-slate-850 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  required
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  value={providerSocials}
                  onChange={(e) => setProviderSocials(e.target.value)}
            ...
            <button
              ...
              className="px-6 py-3.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-soft transition-all duration-200 flex items-center gap-1.5"
            >
              {savingProvider ? "Saving details..." : "Save Provider Credentials"}
            </button>
          </form>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-900 border border-red-200/60 dark:border-red-900/40 rounded-[24px] p-6 shadow-soft space-y-4">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 max-w-2xl leading-relaxed">
          Clearing the system database cache will restore all data models (Program Enrollments,
          Loyalty Ledger, Receipts, Invoices, Expenses, Staff) back to their initial default mock
          templates. Use this button to clear all local edits, adjustments, and test entries. **This
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          >
            ...
            <p className="text-xs text-slate-400 mb-5">
              Define points threshold and reward descriptors.
            </p>
