...
function AdminReports() {
  ...
  const referrersMap: Record<
    string,
    { count: number; name: string; code: string; revenue: number }
  > = {};
  ...
  const sheetsToExport = [
    {
      ...
      desc: "Coaching signups, payment statuses, and agent assignments",
    },
    {
      name: "Health Assessments",
      desc: "BMI logs, step counts, calories targets, and health scores",
    },
    {
      name: "Loyalty Ledger",
      desc: "Historical transactions ledger, points adjustments, and card tiers",
    },
    {
      name: "Invoices",
      desc: "Billing invoice histories, payments schedules, and renewal records",
    },
  ];
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Download raw databases, analyze referral revenues, and check conversions.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-5 shadow-soft">
          ...
          <p className="text-[10px] text-slate-400 mt-1.5">
            Signups generated via member referral codes
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-5 shadow-soft">
          ...
          <p className="text-[10px] text-slate-400 mt-1.5">
            Estimated pipeline value from referrals
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-5 shadow-soft">
          ...
          <p className="text-[10px] text-slate-400 mt-1.5">
            Percentage of referral leads who completed enrollment
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[24px] p-6 shadow-soft space-y-6">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1.5 font-medium">
              Export live database tables instantly to CSV sheets.
            </p>
          </div>
          ...
          <div className="space-y-3.5">
            {sheetsToExport.map((sheet) => (
              <div
                key={sheet.name}
                className="p-3 border border-slate-100 dark:border-slate-800/80 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors flex justify-between items-center gap-3"
              >
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-xs truncate text-slate-900 dark:text-slate-100">
                    {sheet.name}
                  </p>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-[24px] p-6 shadow-soft flex flex-col justify-between">
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800/40">
                  {topReferrers.map((ref, idx) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-3 font-mono text-emerald-600 dark:text-emerald-400 font-bold">
                        {ref.code}
                      </td>
                      <td className="py-4 px-3 text-center font-bold text-slate-700 dark:text-slate-300">
                        {ref.count}
                      </td>
                      <td className="py-4 px-3 text-right font-bold text-slate-900 dark:text-slate-50">
                        ${ref.revenue}
                      </td>
                      <td className="py-4 px-3 text-right text-emerald-500 font-bold">
                        {ref.rate}%
                      </td>
                    </tr>
                  ))}
          ...
          <div className="mt-8 p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 leading-normal flex items-start gap-3">
            ...
            <div className="text-[11px] text-slate-500 dark:text-slate-400">
              <strong>Referral Program Performance Status:</strong> Your patient advocacy leads are
              converting at <strong>82%</strong>, which exceeds clinical baselines by 14%. Referrers
              are actively rewarded +300 points per enrollment automatically.
            </div>
          </div>
        </div>
      </div>
