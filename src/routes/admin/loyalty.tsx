...
function AdminLoyalty() {
  ...
  const getTierBadgeStyle = (tier: string) => {
    switch (tier) {
      ...
      case "Silver":
        return "bg-slate-400/10 text-slate-500 border-slate-200";
      default:
  ...
  const BarcodeRenderer = ({ code }: { code: string }) => {
    ...
    for (let i = 0; i < 45; i++) {
      ...
      lines.push(
        <div
          key={i}
          className={`h-11 ${isBlack ? "bg-slate-800 dark:bg-slate-100" : "bg-transparent"} ${width} shrink-0`}
        />,
      );
    }
    ...
    return (
      <div className="flex flex-col items-center gap-1.5 mt-4 p-2 bg-white/95 rounded-lg border border-slate-200/40 w-fit mx-auto">
        <div className="flex gap-[1.5px] items-center h-11 overflow-hidden">{lines}</div>
        <span className="font-mono text-[8px] font-bold text-slate-500 tracking-[3px] uppercase">
          {code}
        </span>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="font-display font-extrabold text-3xl tracking-tight">
          Loyalty Ledger & Cards
        </h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Manage membership rewards, tier statuses, and adjust loyalty points balances.
        </p>
      </div>
                    ...
                    filteredMembers.map((member: any, idx: number) => (
                      ...
                      >
                        ...
                        <td className="py-4.5 px-4">
                          <span
                            className={`px-2.5 py-0.5 rounded-full border text-[9px] font-black tracking-wider ${getTierBadgeStyle(member["Loyalty Tier"])}`}
                          >
                            {member["Loyalty Tier"] || "Bronze"}
                          </span>
                        </td>
                        <td className="py-4.5 px-4 font-mono font-bold text-emerald-600 dark:text-emerald-400">
                          {member["Referral Code"] || "N/A"}
                        </td>
                        <td className="py-4.5 px-4">
            ...
            {selectedCardClient ? (
              <div className="space-y-6 animate-scale-up">
                ...
                >
                  ...
                  {!cardFlip ? (
                    ...
                    <div className="h-full flex flex-col justify-between relative z-10">
                      ...
                      <div>
                        ...
                        <p className="font-display font-bold text-lg mt-1 truncate">
                          {selectedCardClient.fullName}
                        </p>
                        <p className="text-[8px] text-emerald-100/80 mt-0.5 truncate">
                          {selectedCardClient.programName}
                        </p>
                      </div>
                      ...
                      <div className="flex justify-between items-end border-t border-white/10 pt-3">
                        <div>
                          ...
                          <p className="font-mono text-[9px] font-bold mt-0.5">
                            {selectedCardClient["Enrollment ID"]}
                          </p>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="font-display font-black text-sm text-emerald-350">
                            {selectedCardClient["Loyalty Points"] || 0} pts
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="h-full flex flex-col justify-between relative z-10 text-slate-350 text-[8px] leading-relaxed">
                      ...
                      <BarcodeRenderer
                        code={selectedCardClient["Enrollment ID"] || "OPT-2026-000000"}
                      />
                ...
                <p className="text-center text-[10px] text-slate-400">
                  Click the card to review {cardFlip ? "Front Face" : "Back barcode information"}
                </p>
              ...
              <div className="text-center text-slate-400 py-16 flex flex-col items-center justify-center gap-2">
                ...
                <p className="text-xs">
                  Select a member profile from the list to preview their loyalty card.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-md p-6 shadow-glow z-10 animate-scale-up">
            ...
            <p className="text-xs text-slate-400 mb-5">
              {adjustingClient.fullName} ({adjustingClient["Enrollment ID"]})
            </p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 p-1 bg-slate-100 dark:bg-slate-950 rounded-xl border">
                ...
                <button
                  ...
                  className={`py-2 text-xs font-bold rounded-lg flex items-center justify-center gap-1 ${
                    adjustType === "deduct"
                      ? "bg-white dark:bg-slate-800 text-red-600 dark:text-red-400 shadow-soft"
                      : "text-slate-500"
                  }`}
                >
